#### Kafka → Bronze đã chạy OK (rows=3, có kafka_ts, ingest_ts, dt).:
-	Bronze (raw, append-only)
-	Silver (clean + dedup + upsert theo partition dt)
-	Governance/Provenance (job_run log + schema snapshot + DQ)

---

# A) STREAMING: Bronze → Silver (dedup + upsert partition dt)

## 1) Tạo file spark/lab/lab_bronze_to_silver_orders_kafka.py
...
## 2) Chạy Silver stream

Mở 1 terminal mới, chạy:

```bash
rm -rf data/silver_kafka/orders_silver checkpoints/silver_orders_kafka data/tmp/silver_orders_upsert
mkdir -p data/silver_kafka/orders_silver checkpoints/silver_orders_kafka data/tmp/silver_orders_upsert

python spark/lab/lab_bronze_to_silver_orders_kafka.py
```

## 3) Test end-to-end
-	Bạn produce thêm vài message vào Kafka (có thể trùng order_id=o1 để thấy dedup/upsert)
-	Bronze job đang chạy sẵn (Kafka → Bronze)
-	Silver job sẽ tự đọc file Bronze mới và upsert.
...
print("rows =", df.count())
df.orderBy("ingest_ts", ascending=False).show(50, truncate=False)
spark.stop()
PY
```
...
# B) GOVERNANCE/PROVENANCE: bạn sẽ “nhìn thấy” gì?

Sau khi Silver chạy 1–2 batch, bạn có 2 bảng governance:
...
from pyspark.sql import SparkSession
spark = SparkSession.builder.getOrCreate()
df = spark.read.parquet("data/gov/job_runs").orderBy("ts", ascending=False)
...
from pyspark.sql import SparkSession
spark = SparkSession.builder.getOrCreate()
df = spark.read.parquet("data/gov/schema_snapshots").orderBy("ts", ascending=False)
...
Đây là “mini Data Catalog + lineage thô” cho lab local.
